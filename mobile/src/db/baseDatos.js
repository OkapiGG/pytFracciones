...
export async function getDb() {
  if (!db) {
    db = await SQLite.openDatabaseAsync('appFraccion');
...
/**
 * Ejecuta SQL y devuelve un objeto "parecido" al WebSQL ResultSet:
 * {
...
export async function executeSql(sql, params = []) {
  ...
  try {
    if (isSelect) {
      const rowsArray = await stmt.allAsync(...params);
      return {
        rows: {
      ...
      const { changes, lastInsertRowId } = await stmt.runAsync(...params);
      return {
        rows: { length: 0, item: () => undefined, _array: [] },
        rowsAffected: changes ?? 0,
        insertId: lastInsertRowId ?? null,
      };
    }
  } finally {
...
/** Crea todas las tablas e índices (idéntico a tu esquema) */
export async function initDb() {
  ...
  await database.execAsync(`
    ...
    CREATE INDEX IF NOT EXISTS idx_prog_user     ON progresoLeccion(idUsuario);
    CREATE INDEX IF NOT EXISTS idx_prog_leccion  ON progresoLeccion(idLeccion);
    CREATE INDEX IF NOT EXISTS idx_ejer_leccion  ON ejercicio(idLeccion);
    CREATE INDEX IF NOT EXISTS idx_int_user      ON intento(idUsuario);
    CREATE INDEX IF NOT EXISTS idx_int_ejercicio ON intento(idEjercicio);
    CREATE INDEX IF NOT EXISTS idx_pista_intento ON pistaIntento(idIntento);
    CREATE INDEX IF NOT EXISTS idx_exp_intento   ON explicacionVista(idIntento);
    CREATE INDEX IF NOT EXISTS idx_ins_user      ON insigniaOtorgada(idUsuario);
    CREATE INDEX IF NOT EXISTS idx_ins_def       ON insigniaOtorgada(idInsigniaDet);
    CREATE INDEX IF NOT EXISTS idx_ajustes_user  ON ajustes(idUsuario);
  `);
}
